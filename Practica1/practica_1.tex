...
\begin{figure}[ht]
    \centering
    \includegraphics[scale=0.45]{supervised-class-balancer}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.50\textwidth}
        \centering
        \includegraphics[scale=0.7]{supervised-class-balancer-int2}
        \caption{Datos balanceados en dos intervalos.}
        \label{fig:supervised-class-balancer-int2}
    \end{minipage}\hfill
    \begin{minipage}{0.50\textwidth}
        \centering
        \includegraphics[scale=0.7]{supervised-class-balancer-int5}
...
\clearpage
\subsubsection{filters/supervised/instance/SpreadSubsample}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.50\textwidth}
        \centering
        \includegraphics[scale=0.45]{supervised-spread-subsample-breast-cancer}
...
Para probar este filtro se utiliza la base de datos \code{breast-cancer} por ser su clase binaria y con una distribución 201-85 (fig. \ref{fig:supervised-spread-subsample-breast-cancer}). En una primera prueba, con los parámetros por defecto, no se ha visto ningún cambio en la base de datos al aplicar el filtro. El primer cambio introducido ha sido el valor 50 en el parámetro \code{maxCount}, lo que ha producido subsampling que ha igualado el nº de patrones de cada clase a 50 (fig. \ref{fig:supervised-spread-subsample-max50}). La segunda prueba ha consistido en fijar de nuevo \code{maxCount} a 0 y colocar un 1 en \code{distributionSpread}. Esta configuración sirve para que la proporción máxima entre las clases sea $1:1$, así que también equilibra la distribución de las clases, eliminando patrones de la clase mayoritaria hasta quedar tantos como tenga la minoritaria (fig. \ref{fig:supervised-spread-subsample-spr1}).
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.50\textwidth}
        \centering
        \includegraphics[scale=0.45]{supervised-spread-subsample-max50}
        \caption{Resultado del filtro con \code{distributionSpread=0} y \code{maxCount=50}.}
        \label{fig:supervised-spread-subsample}
    \end{minipage}\hfill
    \begin{minipage}{0.50\textwidth}
        \centering
        \includegraphics[scale=0.45]{supervised-spread-subsample-spr1}
        \caption{Resultado del filtro con \code{distributionSpread=1} y \code{maxCount=0}.}
...
\begin{figure}[ht]
    ...
    \caption{Resultado del filtro con \code{distributionSpread=1.5}9}
